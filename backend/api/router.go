...
func SetupRouter(cfg *config.ServerConfig, mc *cache.MachineCache, pool *servergrpc.AgentConnPool) *gin.Engine {
	...
	{
		// 登录（无需认证）
		api.POST("/auth/login", views.Login)
		...
		{
			// 用户信息
			protected.GET("/auth/user-info", views.GetUserInfo)
			protected.POST("/auth/logout", views.Logout)
			protected.POST("/auth/change-password", views.ChangePassword)
			protected.PUT("/auth/profile", views.UpdateProfile)

			// Dashboard
			protected.GET("/dashboard/stats", views.GetDashboardStats)
			...
			stageTemplates := protected.Group("/stage-templates")
			{
			...
			// 单阶段执行记录管理
			stageExecutions := protected.Group("/stage-executions")
			{
			...
			globalVars := protected.Group("/global-variables")
			{
			...
			hookTemplates := protected.Group("/hook-templates")
			{
			...
			solutionLibraries := protected.Group("/solution-libraries")
			{
			...
			protected.GET("/executions/:id/stream", views.HandleSSE)

			// Install commands
			protected.GET("/install/command", views.GetInstallCommand)
		}
	}
