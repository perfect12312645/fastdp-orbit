        ...
        <el-form-item label="选择机器">
          <div style="width: 100%;">
            <div style="margin-bottom: 8px; display: flex; gap: 8px;">
              <el-button size="small" @click="selectAllMachines" :disabled="allMachines.length === 0">
                <Icon icon="mdi:checkbox-multiple-marked" :size="14" /> 全选
              </el-button>
              <el-button size="small" @click="formData.machine_ids = []" :disabled="formData.machine_ids.length === 0">
              ...
              <span style="font-size: 12px; color: var(--el-text-color-secondary); line-height: 28px;">
                已选 {{ formData.machine_ids.length }} / {{ allMachines.length }}
              </span>
            </div>
            <el-select-v2
...
function selectAllMachines() {
  formData.value.machine_ids = allMachines.value.map((m) => m.id)
}
